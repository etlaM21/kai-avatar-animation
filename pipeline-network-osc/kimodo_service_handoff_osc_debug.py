...
def load_kimodo_model():
    ...
    print("\n--- EXTRACTING SKELETON HIERARCHY ---")
    if hasattr(skeleton, 'parents'):
        print(f"Parents: {list(skeleton.parents)}")
    elif hasattr(skeleton, 'parent_indices'):
        print(f"Parent Indices: {list(skeleton.parent_indices)}")
    else:
        print("Could not find standard parent attributes. Let's look inside:")
        print(dir(skeleton))
    print("-------------------------------------\n")
...
def stream_motion_data(root_positions, local_rot_mats, fps):
    ...
    print("\n--- DIAGNOSTIC REST POSE CHECK (FRAME 0) ---")
    for check_bone in ["LeftLeg", "RightLeg", "LeftShoulder", "LeftArm", "LeftHand", "RightShoulder", "RightArm", "RightHand"]:
        if check_bone in soma_joint_names:
            idx = list(soma_joint_names).index(check_bone)
            mat = rot_np[0, idx]
            q = R.from_matrix(mat).as_quat()
            print(f"{check_bone}: quat = {q.round(3)}")
    print("--------------------------------------------\n")
...
async def lifespan(app: FastAPI):
    print("============================================================")
    print(f"[Version {SCRIPT_VERSION} DEBUG] Running Kimodo generation service with handoff to UE via OSC")
    load_kimodo_model()
    yield
