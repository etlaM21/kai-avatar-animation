...
def clean_rotation(rot_matrix):
    """SVD-orthogonalize a (possibly slightly noisy) rotation matrix, forcing det=+1
    so we never silently produce a reflection instead of a proper rotation."""
...
def build_frame_payload(pos_np, rot_np, frame_idx, soma_joint_names, M):
    """Builds the OSC payload list for a single frame."""
    payload = []
...
# --- Shared motion state, swapped atomically whenever a new generation completes ---
motion_lock = threading.Lock()
current_motion = {
    "pos_np": None,   # (num_frames, 3)
    "rot_np": None,   # (num_frames, num_joints, 3, 3)
    "fps": FPS,
    "version": 0,     # bumped on every new generation, lets the streaming loop detect a swap
}


def streaming_loop():
    """Runs forever in a background thread, started once at server startup.
    Continuously streams the current motion at its target FPS, looping it
    until a new generation lands (checked once per frame)."""
    ...
    print("[@StreamLoop] Streaming thread started, idling until first generation...")

    while True:
        with motion_lock:
            pos_np = current_motion["pos_np"]
            rot_np = current_motion["rot_np"]
            fps = current_motion["fps"]
            local_version = current_motion["version"]

        if pos_np is None:
            time.sleep(0.1)  # nothing generated yet
            continue
        ...
        while True:
            # If a new generation has landed, break out and reload it immediately
            with motion_lock:
                if current_motion["version"] != local_version:
                    break

            start_time = time.perf_counter()
            payload = build_frame_payload(pos_np, rot_np, frame_idx, soma_joint_names, M_ROOT)
            ...
            frame_idx = (frame_idx + 1) % num_frames  # loop back to the start


def blocking_inference(long_p: str, output_path: str, req_fps: int, req_frames: int, req_steps: int, req_seed: int | None):
    """
    Executes heavy tensor compilation on a sub-thread, then atomically swaps the
    result into current_motion for the streaming loop to pick up.
    """
    ...
    print(f"   [Time Step] Core Tensor Inference ({req_steps} steps, {req_frames} frames): {inf_duration:.2f}s")
    ...
    pos_np = root_pos.cpu().numpy()[0]
    rot_np = rot_mats.cpu().numpy()[0]

    # Step 2: Atomically swap the new motion in — streaming loop picks it up
    # on its next per-frame check (within ~1 frame, i.e. ~33ms at 30fps)
    with motion_lock:
        current_motion["pos_np"] = pos_np
        current_motion["rot_np"] = rot_np
        current_motion["fps"] = req_fps
        current_motion["version"] += 1
...
async def generate_motion(payload: GenerationRequest):
    ...
    try:
        # Run inference safely inside a companion thread; the persistent
        # streaming_loop thread keeps looping the *previous* motion the
        # whole time this is running, so playback never stalls.
        async with _inference_lock:
            inf_time = await asyncio.to_thread(
                blocking_inference,
                payload.prompt,
                output_path,
                runtime_fps,
