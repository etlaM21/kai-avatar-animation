...
def stream_motion_data(root_positions, local_rot_mats, fps):
    print(f"[@Stream] Computing Global Forward Kinematics natively...")
    
    # 1. Use Kimodo's built-in FK to calculate absolute World Space matrices 
    global_joint_rots, _, _ = skeleton.fk(local_rot_mats, root_positions)
    
    # 2. Extract arrays
    global_rot_np = global_joint_rots.cpu().numpy()[0]
    ...
    if hasattr(skeleton, 'bone_order_names'):
        ...
        print("[Error] Cannot extract 'bone_order_names' from skeleton.")
        return

    # SOMA (Y-Up RH) to UE5 (Z-Up LH) World Space Conversion Matrix
    M = np.array([
        [ 0,  0, -1],
        [ 1,  0,  0],
        [ 0,  1,  0]
    ...
    print(f"[@Stream] Beginning Absolute World Space OSC broadcast at {fps} FPS...")
    ...
    for frame_idx in range(num_frames):
        ...
        for joint_idx, kimodo_name in enumerate(soma_joint_names):
            if joint_idx >= global_rot_np.shape[1]: 
                continue
            ...
            # Grab the pre-calculated, absolute global rotation matrix
            world_matrix = global_rot_np[frame_idx, joint_idx]
            
            # SVD Orthogonalization
            u, s, vh = np.linalg.svd(world_matrix)
        ...
        # Send the complete World Space frame
        osc_client.send_message("/kaspar/frame", payload)
        
        # Precision 30 FPS pacing
